...
# Old APIs (still kept for backup – unused)
...
def format_ifsc_output(data):
    ...
    if not clean_data:
        return "❌ No data found for this IFSC."
...
def format_pincode_output(data):
    ...
    clean_data = {
        "status": status, "pincode": pincode, "total_records_found": total_records,
        "delivery_status": delivery_status or "N/A", "district": district or "N/A",
        "division": division or "N/A", "region": region or "N/A",
        "state": state or "N/A", "country": country or "India",
        "developer": "𐙚 𓆩𝘼𝙠𝙖𝙨𝙝 𝙊𝙨𝙞𝙣𝙩𓆪𓂃🧑‍💻🎀⃤"
    }
    out = "**PIN Code Search**\n```json\n"
    out += json.dumps(clean_data, indent=4, ensure_ascii=False)
...
def format_weather_output(data):
    ...
    clean_data = {
        "city": city.get("searched"), "state": city.get("state"), "country": city.get("country"),
...
def format_email_output(data):
    ...
    if isinstance(data, dict):
        ...
        if "results" in data:
            ...
            results = data["data"]
    if not results:
        return f"❌ No data found for email: `{query or 'Unknown'}`"
    clean_data = {
        "success": success, "query": query or "N/A",
...
def format_pan_output(data):
    ...
    if not results:
        if isinstance(data, dict) and "pan" in data:
            ...
            return f"❌ No data found for PAN: `{query or 'Unknown'}`"
    clean_results = []
    for record in results:
...
async def perform_lookup(update, context, lookup_type, input_text):
    ...
    if lookup_type == "number":
        # Auto +91 prefix for 10-digit number
...
async def messenger_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "msg_text":
        ...
        await query.edit_message_text("🖼️ **Send Photo**\n\nSend a photo to broadcast.", reply_markup=get_back_keyboard())
        return
    elif data == "msg_video":
        context.user_data["admin_action"] = "msg_video"
        await query.edit_message_text("🎥 **Send Video**\n\nSend a video to broadcast.", reply_markup=get_back_keyboard())
        return
    elif data == "msg_audio":
        context.user_data["admin_action"] = "msg_audio"
        await query.edit_message_text("🎵 **Send Audio/Song**\n\nSend an audio file to broadcast.", reply_markup=get_back_keyboard())
        return
    elif data == "msg_document":
        context.user_data["admin_action"] = "msg_document"
        await query.edit_message_text("📄 **Send Document**\n\nSend a document to broadcast.", reply_markup=get_back_keyboard())
        return
    elif data == "msg_gif":
        context.user_data["admin_action"] = "msg_gif"
        await query.edit_message_text("🎞️ **Send GIF**\n\nSend a GIF to broadcast.", reply_markup=get_back_keyboard())
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "msg_text":
            ...
            if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
                ...
                if str(target) in data:
                    ...
                    save_data(data)
                    resp = {
                        "giveaway": False, "added_now": amount,
                        "total_coins": new_total, "status": "active",
            ...
            if text.isdigit():
                ...
                save_data(data)
                resp = {
                    "giveaway": True, "added_now": amount,
                    "total_coins": total_added, "status": "active",
                    "developer": "𐙚 𓆩𝘼𝙠𝙖𝙨𝙝 𝙊𝙎𝙄𝙉𝙏 𝘾𝙝𝙖𝙣𝙣𝙚𝙡𓆪"
                }
    ...
    if text == "📱 𝘕𝘶𝘮𝘣𝘦𝘳 𝘓𝘰𝘰𝘬𝘶𝘱":
        await update.message.reply_text("📞 Send a 10-digit mobile number (e.g., 7250385668):")
        ...
        await update.message.reply_text("🔢 Send a Telegram User ID (e.g., 5437510481):")
