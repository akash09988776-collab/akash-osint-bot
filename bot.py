...
CHANNELS = [
    ...
    {"name": "Channel AkashOSINT", "username": "@AkashOSINT", "link": "https://t.me/AkashOSINT"}
]
...
async def is_member(user_id, context):
    for ch in CHANNELS:
        try:
            # For private groups, use invite link or chat_id
            if ch["username"].startswith("+"):
                # Private group - cannot check directly, so we'll handle differently
                # For now, we'll skip private group verification or check via invite link
                continue
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_verified(user_id, context):
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(
            "⚠️ You must be a member of both channels to use this bot.\n"
    ...
    if user_id in ADMIN_IDS and context.user_data.get("admin_action"):
        ...
        if action == "msg_text":
            ...
            # These are handled by media handlers
            pass
    ...
    if text.lower() in ["number info", "number", "phone"]:
        ...
        user_data = get_user_data(user_id)
        await update.message.reply_text(f"🪙 You have {user_data['coins']} coins.")
        ...
        await update.message.reply_text("👑 Owner Menu:", reply_markup=get_owner_menu())
    else:
